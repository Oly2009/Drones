...
function finalizarTrabajo($con, $id_trabajo) {
    ...
    if ($trabajo) {
        ...
        mysqli_query($con, "UPDATE trabajos_tareas SET estado = 'finalizado' WHERE id_trabajo = $id_trabajo");
...
if (isset($_GET['preview']) || isset($_GET['ejecutar'])) {
    ...
    $geojsonPath = '../../agregar/parcelas/' . $trabajo['fichero'];
    ...
    if (isset($_GET['ejecutar'])) {
        ...
        mysqli_query($con, "UPDATE trabajos_tareas SET estado = 'en curso' WHERE id_trabajo = $id_trabajo");
    }
    ?>
    <!DOCTYPE html>
    <html lang="es">
    <head>
        ...
        <style>
            #mapa { height: 500px; margin: 20px auto; border-radius: 10px; border: 2px solid #45f3ff; }
            .titulo { text-align: center; color: #45f3ff; }
            #estado-vuelo { text-align: center; font-weight: bold; margin-top: 10px; color: #45f3ff; }
        </style>
    </head>
    <body>
        ...
        <div style="text-align:center; margin-top: 20px;">
            <a class="btn" href="eje_trabajos.php">⬅ Volver</a>
        </div>
        ...
        <script>
            ...
            if (geojsonData && geojsonData !== 'null') {
                const capa = L.geoJSON(geojsonData, {
                    style: {
                        color: '#45f3ff', weight: 3, opacity: 0.7,
                        fillColor: '#45f3ff', fillOpacity: 0.3
                    }
                }).addTo(mapa);
            ...
            if (ruta.length > 0) {
                ...
                <?php if (isset($_GET['ejecutar'])): ?>
                    ...
                    }, 1000); // Tiempo reducido a 1 segundo
                <?php endif; ?>
            } else {
...
$trabajos_q = "SELECT t.id_trabajo, t.id_parcela, t.id_dron, t.estado_general,
                      p.ubicacion, p.fichero, d.marca,
                      tt.estado as estado_tarea,
                      tar.nombre_tarea
               FROM trabajos t
               JOIN parcelas p ON t.id_parcela = p.id_parcela
               JOIN trabajos_tareas tt ON t.id_trabajo = tt.id_trabajo
               JOIN tareas tar ON tt.id_tarea = tar.id_tarea
               JOIN drones d ON t.id_dron = d.id_dron
               WHERE t.estado_general IN ('pendiente', 'en curso')
               AND tt.estado IN ('pendiente', 'en curso')";
...
<body>
    ...
    <?php if (mysqli_num_rows($resultado) > 0): ?>
        <ul class="lista-trabajos">
            <?php while ($trabajo = mysqli_fetch_assoc($resultado)): ?>
                <li>
                    <div>
                        <span><?= htmlspecialchars($trabajo['ubicacion']) ?> (<?= htmlspecialchars($trabajo['marca']) ?>)</span>
                        <div class="tarea-info">Tarea: <?= htmlspecialchars($trabajo['nombre_tarea']) ?> | Estado: <?= htmlspecialchars($trabajo['estado_tarea']) ?></div>
                    </div>
                    <div class="botones">
                        <a class="btn btn-ver" href="?preview=<?= $trabajo['id_trabajo'] ?>">Ver mapa</a>
                        <a class="btn btn-ejecutar" href="?ejecutar=<?= $trabajo['id_trabajo'] ?>">Ejecutar</a>
                    </div>
                </li>
            <?php endwhile; ?>
        </ul>
    ...
    <?php endif; ?>
    <div style="text-align: center; margin-top: 20px;">
        <a href="../trabajos.php" class="btn">⬅ Volver</a>
    </div>
</body>
</html>